"""
Two-model + per-direction utility evaluation (Denis's formula applied correctly).

Step 1 — prediction:
    Model 1 XGB  -> r_hat
    Model 2 XGB  -> sigma_hat (from squared residuals)

Step 2 — evaluate each stock under BOTH directions:
...
Step 3 — portfolio:
    ...
    - Longs  = top decile of conviction among stocks with r_hat > 0
    - Shorts = top decile of conviction among stocks with r_hat < 0
    - Value-weighted by me, 10 bps transaction fee

Key property vs Denis-as-score:
    Both legs prefer LOW sigma_hat (-gamma*log(sigma) always pulls toward low sigma
    regardless of whether you're long or short), so the short leg no longer
    concentrates on high-vol names. BAB tilt should be neutralised.

Test run: 5 seeds per model, gammas = [0, 0.5, 1, 2, 3, 10].
"""
...
frac_pos = (r_hat_test > 0).mean()
print(f"  fraction r_hat > 0 on test: {frac_pos:.1%}")
...
def per_direction_util(r_hat, sigma_hat, gamma, eps=EPS):
    ret_term = np.log1p(np.abs(r_hat) / eps)
    sig_pen  = -gamma * np.log(sigma_hat)
    util_long  =  np.sign(r_hat) * ret_term + sig_pen
    util_short = -np.sign(r_hat) * ret_term + sig_pen
    return util_long, util_short

def portfolio(df, gamma, fee=TRADING_FEE):
    """Long: top-decile conviction among r_hat>0. Short: top-decile conviction among r_hat<0."""
    ul, us = per_direction_util(df['r_hat'].values, df['sigma_hat'].values, gamma)
    d = df.copy()
    d['util_long'], d['util_short'] = ul, us
    ...
    for date, grp in d.groupby('date'):
        ...
        if len(cand_L) < 10 or len(cand_S) < 10:
            continue
        # NYSE-only breakpoints, same logic as other scripts
...
res_df.to_csv('results/two_model_per_direction_test.csv', index=False, float_format='%.4f')
print("\nSaved: results/two_model_per_direction_test.csv")
