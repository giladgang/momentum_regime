"""
test_hmm_combinations_v2.py
============================
Run only the 120 combinations that include CP_SPREAD (replacing TED which died Jan 2022).
Merge with the 210 non-TED results from the first run, then do Pass 2 on the overall top 15.
"""
...
print("=" * 80)
print("  4-FEATURE HMM SEARCH v2: TED → CP_SPREAD")
print("=" * 80)
...
# CP_SPREAD: Commercial Paper (3-mo financial) minus 3-mo T-bill — modern TED replacement
try:
    ...
    n_valid = panel['CP_SPREAD'].notna().sum()
    print(f"  CP_SPREAD: OK ({n_valid} months)")
except Exception as e:
...
# ── Define the feature pool (11 features: TED replaced by CP_SPREAD) ──────
...
FEATURE_POOL = {
    ...
    'CP_SPREAD_z':  'CP_SPREAD (Funding stress)',
...
# Only combos that include CP_SPREAD (the 120 we need to run)
all_combos = [c for c in combinations(FEAT_NAMES, 4) if 'CP_SPREAD_z' in c]
# Non-CP_SPREAD combos (already run as non-TED combos in v1)
non_cp_combos = [c for c in combinations(FEAT_NAMES, 4) if 'CP_SPREAD_z' not in c]
print(f"\n  CP_SPREAD combos to run: {len(all_combos)}")
print(f"  Non-CP_SPREAD combos (reuse from v1): {len(non_cp_combos)}")
print(f"  Total: {len(all_combos) + len(non_cp_combos)}")

# ═══════════════════════════════════════════════════════════════════════════════
#  HMM FUNCTIONS (same as v1)
# ═══════════════════════════════════════════════════════════════════════════════
...
def fit_hmm_fast(Z_train, Z_test, n_iter=600, n_burnin=200, seed=2201):
    ...
    # Identify panic regime via L2 norm: the state with the more extreme
    # (further from zero) mean vector is panic. Feature-agnostic.
...
# ═══════════════════════════════════════════════════════════════════════════════
#  PASS 1: FAST HMM SCREEN (120 CP_SPREAD combinations only)
# ═══════════════════════════════════════════════════════════════════════════════

print("\n" + "=" * 80)
print(f"  PASS 1: Screening {len(all_combos)} CP_SPREAD combinations (fast HMM, 600 iter)")
print("=" * 80)
...
print(f"\n  Pass 1 complete: {len(screen_results)} valid CP_SPREAD combinations "
      f"in {elapsed_total:.0f}s")

# ═══════════════════════════════════════════════════════════════════════════════
#  ALSO SCREEN THE 210 NON-CP_SPREAD COMBOS (recompute — same features minus TED)
# ═══════════════════════════════════════════════════════════════════════════════

# These are the same 210 combos from v1 that didn't have TED.
# We need to re-run them because the feature pool changed (no TED_z column).
# Actually they're identical — same features, same data. But let's run them fresh
# to ensure consistent comparison.

non_cp_names = [f for f in FEAT_NAMES if f != 'CP_SPREAD_z']
non_cp_combos_actual = list(combinations(non_cp_names, 4))

print(f"\n" + "=" * 80)
print(f"  Screening {len(non_cp_combos_actual)} non-CP_SPREAD combinations")
print("=" * 80)

t_start2 = time.time()

for i, combo in enumerate(non_cp_combos_actual):
    feat_list = list(combo)
    ...
    if (i + 1) % 50 == 0:
        ...
        eta = elapsed / (i + 1) * (len(non_cp_combos_actual) - i - 1)
        print(f"  {i+1}/{len(non_cp_combos_actual)} done  "
              f"({elapsed:.0f}s elapsed, ~{eta:.0f}s remaining)")

elapsed2 = time.time() - t_start2
print(f"\n  Non-CP_SPREAD screening complete in {elapsed2:.0f}s")
...
print(f"\n  TOTAL VALID COMBINATIONS: {len(df_screen)}")
n_cp = df_screen['name'].str.contains('CPS').sum()
print(f"    with CP_SPREAD: {n_cp}")
print(f"    without CP_SPREAD: {len(df_screen) - n_cp}")

print(f"\n  TOP 30 by AUC (crisis detection):\n")
print(f"  {'Rank':<5} {'Features':<50} {'AUC':>6} {'Brier':>6} "
      f"{'Cris':>6} {'Calm':>6} {'D':>6} {'COV%':>5} {'Sep':>5}")
print("  " + "-" * 105)
...
for idx, row in df_screen.head(30).iterrows():
    ...
    print(f"  {idx+1:<5} {row['name']:<50} {row['auc']:>6.3f} {row['brier']:>6.3f} "
...
# ═══════════════════════════════════════════════════════════════════════════════
#  PASS 2: FULL PORTFOLIO TEST (top 15 + baseline)
# ═══════════════════════════════════════════════════════════════════════════════
...
for m in all_methods:
    ...
    if best_name:
        ...
        print(f"  {m:<10}  {best_name:<45}  SR={sr:.3f}  Ret={ar:.1%}  Vol={av:.1%}")
...
if len(sorted_port) > 1:
    ...
    plt.suptitle('Portfolio Sharpe: 4-feature HMM combinations (TED replaced by CP_SPREAD)\n'
                 'Same cross-sectional models (M0/M1/M2), different regime signals',
                 fontsize=13, y=1.02)
    plt.tight_layout()
    fig.savefig('hmm_combo_portfolio_comparison_v2.png', dpi=150)
    plt.close(fig)
    print("\n  Saved: hmm_combo_portfolio_comparison_v2.png")
