\begin{table}[H]
\centering
\small
\begin{tabular}{l r r r r r r}
\toprule
Model & $\alpha$ & Sharpe & Ann.\ Ret & Ann.\ Vol & MDD & Calm / Panic SR \\
\midrule
OLS (no reg.) & --- & $-$0.58 & $-$11.1\% & 17.4\% & $-$82.0\% & $-$0.61 / $-$0.56 \\
Ridge & 0.1 & $-$0.58 & $-$11.1\% & 17.4\% & $-$82.0\% & $-$0.61 / $-$0.56 \\
Ridge & 1 & $-$0.58 & $-$11.1\% & 17.4\% & $-$82.0\% & $-$0.61 / $-$0.56 \\
Ridge & 10 & $-$0.58 & $-$11.1\% & 17.4\% & $-$82.0\% & $-$0.61 / $-$0.56 \\
Ridge & 100 & $-$0.58 & $-$11.0\% & 17.4\% & $-$81.9\% & $-$0.60 / $-$0.56 \\
Ridge & 1{,}000 & $-$0.54 & $-$10.5\% & 17.5\% & $-$80.3\% & $-$0.60 / $-$0.47 \\
\midrule
LR (classification) & --- & $-$0.01 & $-$1.3\% & 15.2\% & $-$42.4\% & $-$0.29 / 0.39 \\
XGB & --- & 1.11 & 21.7\% & 19.5\% & $-$22.8\% & 0.84 / 1.53 \\
\bottomrule
\end{tabular}
\caption{Ridge regression baseline predicting continuous forward returns (same target as XGB). The Sharpe of $-$0.58 is robust across all regularisation strengths.}
\label{tab:ridge}
\end{table}
