...
M1: LR (augmented) & 4.3\% & 17.7\% & 0.32 & -39.3\% & 0.06 & 1.40 & 1.8 \\
...
\medskip
\small
\textbf{Notes:} Long-short (top/bottom decile, NYSE breakpoints), value-weighted, net of 10\,bps one-way costs. $\beta$ is the market beta. NW $t$ uses Newey--West standard errors (6 lags). Test period: January 2011 to November 2025 (167 months). ``Augmented'' adds $\pi^2$ and $\pi \times mom_k$ interaction terms to the logistic regression. $^{*}$\,$p<0.10$; $^{**}$\,$p<0.05$; $^{***}$\,$p<0.01$.
\end{table}