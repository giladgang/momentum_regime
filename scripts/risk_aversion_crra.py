"""
CRRA-style risk-aversion analysis: train XGBoost on Denis's utility target
    y_s = sign(r_fwd) * log(1 + |r_fwd| / eps)  -  gamma * log(sigma_s)
for a range of gamma values, then compare portfolio performance and
SHAP feature importances across risk-aversion levels.

Contrast with the mean-variance target (risk_aversion_analysis.py):
    MV:   y_s = r_fwd - (gamma/2) * sigma_s^2      (CARA, fixed absolute price of variance)
    CRRA: y_s = sign(r) * log(1 + |r|/eps) - gamma * log(sigma)
          (fixed proportional trade-off between return and volatility)

Output:
  - results/risk_aversion_crra_results.csv
  - plots/risk_aversion_crra.png
"""
...
EPS = 0.01         # reference return scale (1% monthly)
SIGMA_FLOOR = 0.01 # floor on sigma to keep log(sigma) finite
...
# Trailing 12-month return variance (same source as MV script)
...
print(f"  sigma: mean={train['sigma'].mean():.3f}  median={train['sigma'].median():.3f}  "
      f"p5={train['sigma'].quantile(0.05):.3f}  p95={train['sigma'].quantile(0.95):.3f}")

# ══════════════════════════════════════════════════════════════════════════════
# 2. Portfolio helper (identical to MV script)
# ══════════════════════════════════════════════════════════════════════════════
...
def crra_target(r, sigma, gamma, eps=EPS):
    """Denis's CRRA-style utility: sign(r)*log(1 + |r|/eps) - gamma*log(sigma)."""
...
for i_gamma, gamma in enumerate(GAMMAS):
    if i_gamma > 0:
        time.sleep(10)
    print(f"\n{'='*60}")
    print(f"  gamma = {gamma}  (eps = {EPS})")
    print(f"{'='*60}")

    score_col = f'score_crra_g{gamma}'
    y_train_g = crra_target(r_train, sigma_train, gamma)

    print(f"  target: mean={y_train_g.mean():+.3f}  std={y_train_g.std():.3f}  "
          f"min={y_train_g.min():+.3f}  max={y_train_g.max():+.3f}")
    ...
    agg = pd.concat([pd.Series({'Momentum (agg)': mom_total}), other]).sort_values(ascending=False)
...
# ══════════════════════════════════════════════════════════════════════════════
# 4. Plot (same layout as MV figure)
# ══════════════════════════════════════════════════════════════════════════════

all_features = shap_by_gamma[GAMMAS[0]].index.tolist()
shap_df = pd.DataFrame({g: shap_by_gamma[g].reindex(all_features) for g in GAMMAS}).fillna(0)
...
ax4.set_xlim(min(GAMMAS), max(GAMMAS))
ax4.set_ylim(0, 100)
...
fig.suptitle(r'XGBoost with CRRA-style Utility Target:  '
             r'$y_s = \mathrm{sign}(r)\log(1 + |r|/\epsilon) - \gamma\log(\sigma_s)$  '
             rf'($\epsilon$ = {EPS})',
             fontsize=14, fontweight='bold', y=0.98)

plt.savefig('plots/risk_aversion_crra.png', dpi=200, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.close(fig)
print("\nSaved: plots/risk_aversion_crra.png")
print("Saved: results/risk_aversion_crra_results.csv")
