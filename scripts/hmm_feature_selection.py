...
# Feature pool: 9 features with full 1990-2025 coverage
FEATURE_POOL = ['DD_z', 'VOL_z', 'DISP_z', 'REL_N_z', 'CS_z', 'LVIX_z',
                'ADR_z', 'SKEW_z', 'TERM_z']
...
# Crisis/calm periods for validation
...
VALIDATION_PERIODS = [
    ...
    ('Non-crisis',   '2003-01-01', '2006-12-31', 'calm',   0.4),
...
def load_data():
    """Load and prepare all data. Called once at startup."""
    print("Loading data ...")
...
def run_pass1(panel, available, all_combos):
    ...
    df.to_csv('hmm_feature_selection_pass1.csv', index=False)
...
def run_pass2(panel, train_stocks, test_stocks, passed_combos):
    ...
    df = pd.DataFrame(results)
    df.to_csv('hmm_feature_selection_pass2.csv', index=False)
...
def run_pass3(panel, train_stocks, test_stocks, top_combos):
    ...
    df = pd.DataFrame(results)
    df.to_csv('hmm_feature_selection_pass3.csv', index=False)
    ...
    print(f"  Saved: hmm_feature_selection_pass3.csv")
    return results
...
def run_pass4(panel, train_stocks, test_stocks, top_combos):
    ...
    df.to_csv('hmm_feature_selection_pass4.csv', index=False)
    print(f"\n  Saved: hmm_feature_selection_pass4.csv")
    return results
...
def main():
    parser = argparse.ArgumentParser(description='HMM Feature Selection Pipeline')
    ...
    if args.run_pass in (0, 1):
        ...
        df = pd.read_csv('hmm_feature_selection_pass1.csv')
    ...
    if args.run_pass in (0, 2):
        ...
        df = pd.read_csv('hmm_feature_selection_pass2.csv')
