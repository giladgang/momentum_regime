"""
zscore_combined_heatmap.py
==========================
Side-by-side heatmaps: long-only z-score, short-only z-score, and long minus
short spread, with a shared pi_filter line panel on the left.

Outputs:
- results/zscore_short_by_month.csv          (new)
- plots/zscore_combined_heatmap.png / .pdf
- plots/zscore_combined_heatmap.html
"""
...
# ── Monthly z-scores per horizon: long, short, spread ──
horizons = list(range(1, 13))
long_rows, short_rows, spread_rows = [], [], []
for date, grp in test.groupby('date'):
    rl = {'date': date}
    rs = {'date': date}
    rd = {'date': date}
    ...
    for h in horizons:
        ...
        if s > 0 and long_mask.any() and short_mask.any():
            ...
            rl[f'mom_{h}'] = z_long
            rs[f'mom_{h}'] = z_short
            rd[f'mom_{h}'] = z_long - z_short
        else:
            rl[f'mom_{h}'] = np.nan
            rs[f'mom_{h}'] = np.nan
            rd[f'mom_{h}'] = np.nan
    long_rows.append(rl)
    short_rows.append(rs)
    spread_rows.append(rd)

long_df = pd.DataFrame(long_rows).sort_values('date').reset_index(drop=True)
...
z_spread = spread_df[[f'mom_{h}' for h in horizons]].values
dates = long_df['date']

# Shared symmetric colour scale across the three panels so they're comparable.
vmax = float(max(
    np.nanmax(np.abs(z_long)),
    np.nanmax(np.abs(z_short)),
    np.nanmax(np.abs(z_spread)),
))
...
fig, axes = plt.subplots(
    1, 4, figsize=(20, 14),
    gridspec_kw={'width_ratios': [0.22, 1, 1, 1], 'wspace': 0.06},
    sharey=True,
)
ax_pi, ax_long, ax_short, ax_ls = axes
...
_draw(ax_short, z_short, 'Short leg')
im = _draw(ax_ls, z_spread, 'Long − short')
...
fig.suptitle('Cross-sectional z-score of selected stocks by month, horizon, and leg',
             fontsize=14, fontweight='bold', y=0.995)

cbar = fig.colorbar(im, ax=axes.tolist(), shrink=0.85, pad=0.02)
cbar.set_label('z-score', fontsize=10)

fig.savefig('plots/zscore_combined_heatmap.png', dpi=150, bbox_inches='tight')
fig.savefig('plots/zscore_combined_heatmap.pdf', bbox_inches='tight')
plt.close(fig)
print('Saved: plots/zscore_combined_heatmap.png')
print('Saved: plots/zscore_combined_heatmap.pdf')

# ── Interactive HTML ──
date_labels = dates.dt.strftime('%Y-%m').tolist()
fig2 = make_subplots(
    rows=1, cols=4,
    column_widths=[0.14, 0.28, 0.28, 0.30],
    shared_yaxes=True,
    horizontal_spacing=0.015,
    subplot_titles=('Panic prob. π', 'Long leg z', 'Short leg z', 'Long − short z'),
)
...
for col_idx, (z_mat, label) in enumerate(
    [(z_long, 'Long z'), (z_short, 'Short z'), (z_spread, 'L−S z')], start=2
):
    fig2.add_trace(
        go.Heatmap(
            z=z_mat, x=horizons, y=date_labels,
            colorscale='RdBu_r', zmin=-vmax, zmax=vmax,
            colorbar=dict(title='z-score') if col_idx == 4 else None,
            showscale=(col_idx == 4),
...
fig2.update_layout(
    title='Cross-sectional z-score by month × horizon: long, short, and spread',
    width=1500, height=1200,
    margin=dict(l=80, r=40, t=80, b=60),
)
fig2.write_html('plots/zscore_combined_heatmap.html', include_plotlyjs='cdn')
print('Saved: plots/zscore_combined_heatmap.html')
