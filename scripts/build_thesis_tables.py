...
def render_table_factor_alphas(metrics):
    ...
    lines += [
        r'\bottomrule',
        r'\end{tabular}',
        r"\caption{Factor model regressions for XGB (XGBoost, mom+$\\pi$). $\alpha$ is annualised. $t$-statistics use Newey--West standard errors (6 lags). Factor loadings shown in the wide table generated by scripts/new_ls_analyses.py.}",
...
def render_table_international_results(metrics):
    """International robustness table: UK + JP x {Local market, Fixed 12-mo, XGB}
