"""
cluster_k4_descriptor_table.py
-------------------------------
Compute the consolidated per-cluster descriptor table for the K=4 aggregate
L2 z-score clusters.  Feeds §5.2 of the master's thesis.

Inputs
------
- results/thesis/zscore_l2_k4_labels.csv      167 rows (date, cluster)
- results/thesis/zscore_l2_k4_centroids.csv   existing centroids for cross-check
...
Outputs
-------
- results/thesis/cluster_k4_descriptor_table.csv   4-row master table
- results/thesis/cluster_k4_member_dates.csv        167-row long-form dates
- results/thesis/cluster_k4_representative_dates.csv 20-row closest-to-centroid dates
- results/thesis/cluster_k4_year_frequency.csv       year x cluster pivot

Usage
-----
    python scripts/cluster_k4_descriptor_table.py
"""
...
OUT_MASTER    = RES_DIR / "cluster_k4_descriptor_table.csv"
OUT_MEMBERS   = RES_DIR / "cluster_k4_member_dates.csv"
OUT_REPS      = RES_DIR / "cluster_k4_representative_dates.csv"
OUT_YEARFREQ  = RES_DIR / "cluster_k4_year_frequency.csv"
...
# Verify cluster ordering is by centroid_mean descending
assert list(descriptor_df["z_centroid_mean"]) == sorted(
    descriptor_df["z_centroid_mean"], reverse=True
), "Clusters are NOT ordered by centroid_mean descending — check labelling!"
...
# Ensure all 4 cluster columns exist
for cl in range(K):
    col = f"cluster_{cl}"
...
# V3: Cluster labels are 0..3
cluster_set = set(labels_df["cluster"].unique())
check(cluster_set == {0, 1, 2, 3},
      "Cluster labels are 0..3",
      f"set(clusters) = {cluster_set}, expected {{0,1,2,3}}")
...
for cl in range(K):
    ...
    check(max_diff <= 1e-6,
          ...
          f"max abs diff across 12 dims = {max_diff:.2e}, threshold 1e-6")
...
# V13: Representative dates: exactly 20 rows, no duplicate (cluster, rank)
n_rep_rows = len(rep_df)
check(n_rep_rows == 20,
      "Representative dates: 20 rows total",
      f"got {n_rep_rows}, expected 20")
...
for path, name in [
    (OUT_MASTER,   "cluster_k4_descriptor_table.csv"),
    (OUT_MEMBERS,  "cluster_k4_member_dates.csv"),
    (OUT_REPS,     "cluster_k4_representative_dates.csv"),
    (OUT_YEARFREQ, "cluster_k4_year_frequency.csv"),
]:
    check(path.exists(),
