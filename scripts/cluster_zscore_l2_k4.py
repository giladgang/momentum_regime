"""
cluster_zscore_l2_k4.py
------------------------
Pure-L2 KMeans K=4 on the full 167-month 12-d z-curve panel (no regime pre-split).

This is the data-driven counterpart to the within-regime 2x2 grid.
No preprocessing, no PCA, no standardisation -- pure Euclidean distance on
the raw z-curves.

Outputs (results/thesis/):
  zscore_l2_k4_labels.csv
  zscore_l2_k4_centroids.csv
  zscore_l2_k4_vs_fingerprint_within_regime_confusion.csv
  zscore_l2_k4_vs_l2_within_regime_confusion.csv

Plot (plots/thesis/):
  zscore_l2_k4_dispersion.png
  zscore_l2_k4_dispersion.pdf
"""
...
L2_WR_CSV          = ROOT / "results/thesis/zscore_l2_within_regime_labels.csv"
...
sizes_ordered = [int((labels_ordered == k).sum()) for k in range(K)]
...
centroids_path = RES_DIR / "zscore_l2_k4_centroids.csv"
...
labels_path = RES_DIR / "zscore_l2_k4_labels.csv"
...
# ---------------------------------------------------------------------------
# 11. Confusion vs fingerprint within-regime
# ---------------------------------------------------------------------------
print("\nBuilding confusion matrix vs fingerprint within-regime ...", flush=True)
fp_wr = pd.read_csv(FINGERPRINT_WR_CSV, parse_dates=["date"])

merged_fp = label_df.merge(
    fp_wr[["date", "combined_label"]].rename(columns={"combined_label": "fp_label"}),
    on="date", how="inner"
)
print(f"  Merged rows: {len(merged_fp)}", flush=True)

FP_CELLS = ["calm.0", "calm.1", "panic.0", "panic.1"]
conf_fp = pd.crosstab(
    merged_fp["cluster"],
    merged_fp["fp_label"],
    rownames=["l2_k4_cluster"],
    colnames=["fingerprint_within_regime"],
)
for c in FP_CELLS:
    if c not in conf_fp.columns:
        conf_fp[c] = 0
conf_fp = conf_fp[FP_CELLS]
for cl_idx in range(K):
    if cl_idx not in conf_fp.index:
        conf_fp.loc[cl_idx] = 0
conf_fp = conf_fp.loc[sorted(conf_fp.index)]

conf_fp_path = RES_DIR / "zscore_l2_k4_vs_fingerprint_within_regime_confusion.csv"
conf_fp.to_csv(conf_fp_path)
print(f"  Saved: {conf_fp_path}", flush=True)
print(conf_fp.to_string(), flush=True)

ari_vs_fp = adjusted_rand_score(merged_fp["cluster"].values, merged_fp["fp_label"].values)
print(f"\n  ARI (L2 K=4 vs fingerprint within-regime) = {ari_vs_fp:.4f}", flush=True)

# ---------------------------------------------------------------------------
# 12. Confusion vs L2 within-regime
# ---------------------------------------------------------------------------
print("\nBuilding confusion matrix vs L2 within-regime ...", flush=True)
l2_wr = pd.read_csv(L2_WR_CSV, parse_dates=["date"])

merged_l2 = label_df.merge(
    l2_wr[["date", "combined_label"]].rename(columns={"combined_label": "l2_wr_label"}),
    on="date", how="inner"
)
print(f"  Merged rows: {len(merged_l2)}", flush=True)

L2_WR_CELLS = ["calm-l2.0", "calm-l2.1", "panic-l2.0", "panic-l2.1"]
conf_l2 = pd.crosstab(
    merged_l2["cluster"],
    merged_l2["l2_wr_label"],
    rownames=["l2_k4_cluster"],
    colnames=["l2_within_regime"],
)
for c in L2_WR_CELLS:
    if c not in conf_l2.columns:
        conf_l2[c] = 0
conf_l2 = conf_l2[L2_WR_CELLS]
for cl_idx in range(K):
    if cl_idx not in conf_l2.index:
        conf_l2.loc[cl_idx] = 0
conf_l2 = conf_l2.loc[sorted(conf_l2.index)]

conf_l2_path = RES_DIR / "zscore_l2_k4_vs_l2_within_regime_confusion.csv"
conf_l2.to_csv(conf_l2_path)
print(f"  Saved: {conf_l2_path}", flush=True)
print(conf_l2.to_string(), flush=True)

ari_vs_l2 = adjusted_rand_score(merged_l2["cluster"].values, merged_l2["l2_wr_label"].values)
print(f"\n  ARI (L2 K=4 vs L2 within-regime) = {ari_vs_l2:.4f}", flush=True)

# ---------------------------------------------------------------------------
# 13. Plot: 4-panel dispersion
# ---------------------------------------------------------------------------
...
for ext in ("png", "pdf"):
    out_path = PLOT_DIR / f"zscore_l2_k4_dispersion.{ext}"
...
# 14. Open PNG
# ---------------------------------------------------------------------------
subprocess.run(["open", str(PLOT_DIR / "zscore_l2_k4_dispersion.png")], check=False)

# ---------------------------------------------------------------------------
# 15. HEADLINE
# ---------------------------------------------------------------------------
print("\n" + "=" * 60, flush=True)
print("=== L2 K=4 aggregate ===", flush=True)
print("=" * 60, flush=True)
print(f"ARI stability: {mean_ari_stability:.4f} (mean pairwise across {len(STABILITY_SEEDS)} seeds)", flush=True)
print(f"\nPer cluster (ordered by overall z desc):", flush=True)
...
print(conf_fp.to_string(), flush=True)
print(f"ARI = {ari_vs_fp:.4f}", flush=True)

print(f"\nConfusion vs L2 within-regime:", flush=True)
print(conf_l2.to_string(), flush=True)
print(f"ARI = {ari_vs_l2:.4f}", flush=True)

# Interpretation
if ari_vs_fp > 0.7 and ari_vs_l2 > 0.7:
    interpretation = (
        "L2 K=4 strongly recovers the within-regime 2x2 grid, suggesting that "
        "regime membership and sub-regime z-level are the dominant axes of z-curve "
        "variation -- the data-driven 4-cluster solution collapses onto the same "
        "partition whether or not regime is pre-specified."
    )
elif ari_vs_fp > 0.4 or ari_vs_l2 > 0.4:
    interpretation = (
        "L2 K=4 partially overlaps with the within-regime 2x2 cells but finds a "
        "different cut. The unconstrained clustering captures some of the same z-curve "
        "structure as the regime-conditioned approach, but the overall partition is "
        "meaningfully distinct -- regime membership is not the only axis of variation."
    )
else:
    interpretation = (
        "L2 K=4 finds a largely different partition than the within-regime 2x2 cells. "
        "The unconstrained clustering organises months along a different axis than "
        "regime membership + sub-regime z-level, suggesting the z-curves contain "
        "additional structure beyond the regime-conditioned 2x2 grid."
    )
...
print(f"  Confusion (fingerprint) CSV exists: {conf_fp_path.exists()}", flush=True)
print(f"  Confusion (l2-within) CSV exists: {conf_l2_path.exists()}", flush=True)
for ext in ("png", "pdf"):
    p = PLOT_DIR / f"zscore_l2_k4_dispersion.{ext}"
