...
def main():
    ...
    steps = {
        ...
        # ── Auxiliary thesis-cited generators added 2026-05-03 after a
        # provenance audit found their outputs (plots/thesis/zscore_panic_subtypes.png
        # and tables/table_leg_betas.tex) were not regenerated by the canonical
        # pipeline run — same silent-staleness pattern as the 2026-04-27 incident.
        # Both depend on Steps 1+2 (read artefacts/cs_artefacts_data.pkl).

        36: ('scripts/panic_subtype_analysis.py',
             'STEP 36: Panic-month sub-type decomposition '
             '(plots/thesis/zscore_panic_subtypes.png + tables/table_panic_subtypes.tex). '
             'Depends on Steps 1+2 (cs_artefacts_data.pkl + panel_with_regimes.parquet)'),

        37: ('scripts/leg_betas_by_regime.py',
             'STEP 37: Leg-level CAPM betas by regime, M2 vs fixed-12mom '
