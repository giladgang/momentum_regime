...
This section combines ideas and statistical methods from the literatures reviewed in Section~\ref{sec:literature} into a two-stage framework that tests the research question posed in Section~\ref{sec:introduction}. Stage~1 estimates a Bayesian two-state Hidden Markov Model on a single macrofinancial stress indicator, the market drawdown, producing a continuous probability of the panic state. Stage~2 combines that probability with cross-sectional momentum inside a nonlinear stock-selection model. The case for conditioning on drawdown alone, grounded in the momentum-crash literature and confirmed by a selection rule that uses training information only, is developed in Section~\ref{sec:feature_selection}.
...
The Gibbs sampler is re-estimated on the expanding training window each year (Section~\ref{sec:hmm_output}). To reduce sensitivity to MCMC initialization, the full sampler is run independently with $N_s = 50$ different random seeds. Each chain is run for $M=2{,}000$ iterations with the initialization described above. The first $B=500$ iterations are discarded as burn-in, leaving $1{,}500$ posterior draws per seed.
...
\end{equation}
Averaging across MCMC chains is a methodological requirement, not a performance optimization. A single Gibbs chain produces a noisy regime signal whose downstream Sharpe varies meaningfully across random initializations; averaging across chains reduces this noise, and the variance shrinks as more chains are added. By 50 chains the downstream Sharpe has effectively stabilized; the seed-convergence analysis for this sampler places the plateau well below that count (Appendix~\ref{app:seed_convergence}), and the applied ensemble uses a 50-chain budget that sits comfortably above it. All out-of-sample numbers reported below are computed from this 50-chain ensemble, not from a single chain.
