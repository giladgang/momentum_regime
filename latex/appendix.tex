...
\section{Additional Statistical Tests}\label{app:statistical_tests}

\subsection{Information Coefficient Analysis}\label{app:ic}

Table~\ref{tab:ic} reports the monthly information coefficients (Spearman rank correlations) between each model's stock scores and realised forward returns. The logistic regression (M1) achieves the highest mean IC (0.054), which is statistically significantly greater than momentum alone ($t = 2.78$, $p < 0.01$). Both M1 and momentum exhibit higher IC in calm months than in panic months, consistent with the finding that calm-period stock selection drives the overall performance improvement.

\input{tables/table_ic}

\subsection{Granger Causality Tests}\label{app:granger}

Table~\ref{tab:granger} reports Granger causality tests between the regime signal and momentum's cross-sectional predictive power. The regime signal Granger-causes momentum IC at all lag specifications tested (significant at the 10\% level for lags 1--2, at the 5\% level for lag 3), providing evidence that the regime signal contains leading information about when momentum will be effective.

\input{tables/table_granger}

\section{FFBS Derivation}\label{app:ffbs}

This appendix provides a detailed derivation of the Forward-Filtering Backward-Sampling (FFBS) algorithm used in Step~1 of the Gibbs sampler (Section~\ref{sec:gibbs_sampler}).

\subsection{Forward Filtering}

The goal of the forward pass is to compute, for each time step $t$, the filtered probability $\alpha_t(k) = P(s_t = k \mid \mathbf{z}_{1:t})$ for each state $k \in \{0,1\}$.
...
Given the filtered probabilities $\{\alpha_t(k)\}_{t=1}^T$, the backward pass samples a complete state path $(s_1, \dots, s_T)$ from the posterior $P(\{s_t\} \mid \mathbf{z}_{1:T})$.
...
\end{equation}
Normalising gives the sampling distribution in Equation~(\ref{eq:backward_sampling}). The key insight is that the backward pass needs only the filtered probabilities (from the forward pass) and the transition matrix, not the raw observations. Each sampled state $s_t$ depends on the future path only through $s_{t+1}$, preserving the Markov property.
