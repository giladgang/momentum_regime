...
All portfolios are rebalanced monthly: both the long and short legs are reconstituted every month using updated scores. Monthly rebalancing is essential for the regime-conditional strategy, as the holding period analysis in Section~\ref{sec:economic_mechanism} shows that the reversal premium exploited during panic months dissipates within one month.
...
The Sharpe ratio ranges from 0.76 to 0.97 across all configurations tested. Performance is robust to moderate perturbations in tree depth (0.90--0.95) and ensemble size (0.90--0.97). Only the aggressive learning rate of 0.10 degrades performance substantially (Sharpe 0.76), consistent with overfitting to training-period patterns.
...
In the baseline split (1990--2010 training, 2011--2025 test), M2 achieves a Sharpe of 0.97 while M1 collapses (0.05), confirming the nonlinear regime-momentum interaction documented in Section~\ref{sec:performance_results}.
...
Removing the regime signal entirely reduces M2's Sharpe from 0.97 to 0.48, a drop of 0.49. The regime signal is essential for long-short momentum stock selection, not merely a marginal improvement.
...
The real HMM signal roughly doubles the Sharpe ratio (0.97 vs.\ 0.48 without), confirming that the regime signal's contribution is specific and economically meaningful.
...
\subsection{Stress Test}\label{app:stress_test}

M2's return distribution has positive skewness (+0.81), meaning the strategy is not crash-prone despite its use of reversal bets. The 95\% Value-at-Risk is $-$5.0\% per month, with a Conditional Value-at-Risk (expected shortfall) of $-$8.0\%. The longest losing streak is 6 months. A bootstrap simulation of prolonged bear markets (6 to 24 consecutive panic months) indicates potential cumulative losses, but the positive skewness and limited drawdown duration suggest that the strategy is more resilient than a static long-short momentum portfolio, which suffers from severe negative skewness \citep{DanielMoskowitz2016}.
