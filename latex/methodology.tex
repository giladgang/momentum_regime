...
\end{equation}
For row 0 (calm), the prior pseudo-counts are $(\alpha_{00}, \alpha_{01}) = (9, 1)$ from Equation~\eqref{eq:dirichlet_prior}, so the posterior becomes $\text{Dir}(9 + 140,\; 1 + 10) = \text{Dir}(149,\; 11)$. The expected persistence probability under this posterior is $149/160 = 0.93$, reflecting that the data reinforced the prior belief in regime persistence. A new transition matrix row is then drawn from this Dirichlet distribution. Row 1, the panic-to-calm and panic-to-panic probabilities $(P_{10}, P_{11})$, is updated by the same procedure but with the symmetric Dir$(1, 9)$ prior, yielding an expected panic persistence of roughly $0.9$.
