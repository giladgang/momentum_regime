...
\end{equation}
A new pair of transition probabilities is then drawn from each Dirichlet posterior, and the two rows are stacked to give the updated transition matrix $\mathbf{P}$ that will be held fixed during the next iteration's state-path sampling. Because the prior is strongly tilted toward persistence, the sampled $\mathbf{P}$ tracks the data while staying anchored around a persistent diagonal even on short or noisy paths.
