...
In a two-state HMM, the labels $0$ and $1$ are intrinsically arbitrary. Relabeling the states leaves the likelihood unchanged. The HMM therefore identifies two clusters in the $D$-dimensional feature space without knowing which one should be called panic. To break the symmetry, the feature behavior during externally-defined historical crisis windows is used as a reference: each feature receives an expected sign during stress (the direction in which it moves during a crisis relative to calm periods), and the cluster whose posterior mean vector best aligns with that pattern is designated as ``panic'' and the other as ``calm''. The specific windows and sign assignments are listed in Appendix~\ref{app:label_switching}. If the Gibbs sampler assigns the panic characteristics to state~$0$ rather than state~$1$, the labels are swapped so that $\pi_t^{\text{filter}}$ always represents the probability of the panic state. The crisis windows enter only at this naming step. The HMM identifies its two clusters from the emission features without ever seeing the windows; the windows are used solely to decide which of the two emergent clusters carries the ``panic'' label.

A caveat follows from this naming convention. The HMM identifies two regimes by clustering on the emission features, and the label ``panic'' is then assigned to the cluster whose posterior mean aligns with crisis directions. The label therefore reflects a statistical resemblance to historical crisis periods rather than a directly observed market state.
...
\end{equation}
A value near $0$ indicates high confidence in calm, near $1$ indicates panic, and intermediate values reflect genuine ambiguity. This filtered probability serves as the regime signal passed to the portfolio construction methods in Section~\ref{sec:methods}.
