...
The empirical framework is two stages. The first stage estimates market regimes using a Bayesian Hidden Markov Model (Section~\ref{sec:stage1}): a two-state HMM is fit to four macrofinancial stress indicators via Gibbs sampling, producing a monthly filtered probability $\pi_t^{\text{filter}}$ that the market is in a panic state. The HMM is a probabilistic projection of the four-dimensional stress trajectory onto a one-dimensional regime axis: $\pi_t^{\text{filter}} = P(s_t = 1 \mid \mathbf{z}_{1:t})$ collapses the joint behaviour of all four indicators up to month $t$ into a single scalar. Because this probability conditions only on information available at time $t$, it is a real-time signal with no look-ahead bias.
