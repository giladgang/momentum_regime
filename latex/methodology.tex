...
\end{equation}
For the calm-to-X transition probabilities the prior pseudo-counts are $(9, 1)$ from Equation~\eqref{eq:dirichlet_prior}, so the posterior becomes $\text{Dir}(9 + 140,\; 1 + 10) = \text{Dir}(149,\; 11)$. The expected calm-persistence probability under this posterior is $149/160 = 0.93$, reflecting that the data reinforced the prior belief in regime persistence. A new pair of transition probabilities is then drawn from this Dirichlet distribution. The panic-to-X transition probabilities are updated by the same procedure with the symmetric $\text{Dir}(1, 9)$ prior, yielding an expected panic persistence of roughly $0.9$.
