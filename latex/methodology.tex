...
A maximum depth of 4 allows the model to capture up to four-way feature interactions. The choice of depth is consequential: depth~1 permits no interactions between features, depth~2 permits only pairwise interactions (e.g., one split on $\pi_t^{\text{filter}}$ followed by one split on a single momentum horizon), and depth~$\geq 3$ permits higher-order conditioning across multiple horizons simultaneously. Table~\ref{tab:xgb_hyperparams} and Figure~\ref{fig:depth_sharpe} show that performance increases monotonically from depth~1 (Sharpe~0.46) through depth~4 (Sharpe~1.08), confirming that the regime-momentum interaction requires multi-dimensional conditioning that cannot be captured by pairwise splits alone.
