...
**Update 2026-07-05:** rerun against the current production artefacts (May 7) gives OLS/Ridge Sharpe -0.58, Ann Ret -11.1%, MDD -82.0% (table numbers above are the April run; `tables/table_ridge.tex` carries the current values). The plain OLS result was promoted to `tab:performance` (see the OLS baseline row entry below). Per Gilad's decision the ridge sweep table stays a diagnostic artifact and is NOT included in the thesis (briefly added to the appendix, then reverted same day). Generator fixes retained: caption Sharpe computed from data (stale $-$0.57 fixed), LR row label cleaned to "LR (classification)". `tests/test_table_ridge.py` guards tex/csv consistency and asserts table_ridge is not referenced from any thesis .tex.

### OLS baseline row (`table_performance`, `results/thesis/ols_monthly_returns.csv`)

Plain OLS (linear regression, continuous forward-return target — same target as XGB, same production-preprocessed features X_tr_s/X_te_s as LR) evaluated in the standard long-short construction and added as a row to `tab:performance`. Generated by `scripts/ols_baseline_row.py` (Step 22, runs after Step 3; insertion is idempotent).

| Model | Ann Ret | Ann Vol | Sharpe | MDD | Beta | NW t | Final $ |
|---|---:|---:|---:|---:|---:|---:|---:|
| OLS | -11.1% | 17.4% | -0.58 | -82.0% | 0.27 | -2.25** | 0.2 |

OLS on the continuous target loses money at statistical significance (NW t = -2.25, p = 0.024). With n = 1,161,014 training rows and p = 13 features there is no estimation variance for regularisation to fix; the linear model's failure is pure functional-form bias (pi_filter is constant within each month, so a linear score cannot reorder the cross-section). Completes the ablation ladder in the main table: OLS -0.58 (linear, regression target) / LR -0.01 (linear, classification target) / XGB 1.11 (nonlinear).
